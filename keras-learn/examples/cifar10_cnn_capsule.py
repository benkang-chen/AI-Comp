...
class Capsule(Layer):
    ...
    def build(self, input_shape):
        """
         这是你定义权重的地方。这个方法必须设 self.built = True，可以通过调用 super([Layer], self).build() 完成
        :param input_shape:
        :return:
        """
        ...
        if self.share_weights:
            ...
            self.kernel = self.add_weight(
                ...
                initializer='glorot_uniform',
                trainable=True)
        super(Capsule, self).build(input_shape)  # 一定要在最后调用它

    def call(self, inputs):

        """
        这里是编写层的功能逻辑的地方。你只需要关注传入 call 的第一个参数：输入张量，除非你希望你的层支持masking。
